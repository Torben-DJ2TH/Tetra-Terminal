...
export async function initTalkGroupDisplay(getContacts) {
  const container = document.getElementById('tgDisplay');
  if (!container) return;
...
export function markGroupActive(tgId, issi) {
  const el = document.getElementById(`tg-${tgId}`);
  if (!el) return;
