// Functions to fetch contacts from Terminal and store in IndexedDB
...
export async function fetchContacts() {
  ...
  for (let startIdx = 1; startIdx <= maxIndex; startIdx += 100) {
    ...
    await sendCommand(`AT+CPBR=${startIdx},${endIdx}`);
  ...
  if (contacts.length) {
    ...
    print(`✅ ${contacts.length} Kontakte gespeichert`);
  } else {
    print('⚠️ Keine Kontakte gelesen');
  }
}
